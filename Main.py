...
class enemy():
    def __init__(self,pos,dmg,dfn,hp,size,name,v,cooldown,drops,Type,level):
        self.dmg = dmg+dmg*level/3
        self.pos = pos
        self.dfn = dfn+dfn*level/3
        self.hp = hp+hp*level/3
        self.size = size
        self.name = name
        self.v = v+dmg*level/3
...
def add(level):
    ...
    enemies.put(enemy([randint(30,370),randint(30,370)],enemTypes[a][0],enemTypes[a][1],enemTypes[a][2],enemTypes[a][3],enemTypes[a][4],enemTypes[a][5],enemTypes[a][6],enemTypes[a][7],enemTypes[a][8],level))
...
while not done:
    ...
    if mode == 0:
        ...
        if pygame.time.get_ticks() >= lastspawn + spawn:
            ...
            wave+=1
        if player.xp >= pow(2,player.lvl):
            player.lvl+=1
            player.xp = 0
            player.maxhp+=round(pow(player.lvl,1.95))
            player.hp+=round(pow(player.lvl,1.95))
    elif mode == 4:
        player.drawInv()
        for i in range(len(player.inv)):
